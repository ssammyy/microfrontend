...
@Entity
@Table(name = "DAT_KEBS_MAIN_RFC_COI_ITEMS")
class MainRfcCoiItemsEntity : Serializable{
    @Column(name = "ID")
    @SequenceGenerator(name = "DAT_KEBS_MAIN_RFC_COI_ITEMS_SEQ_GEN", sequenceName = "DAT_KEBS_MAIN_RFC_COI_ITEMS_SEQ", allocationSize = 1)
    @GeneratedValue(generator = "DAT_KEBS_MAIN_RFC_COI_ITEMS_SEQ_GEN", strategy = GenerationType.SEQUENCE)
    @Id
    var id: Long = 0
    ...
    @JoinColumn(name = "RFC_COI_ID", referencedColumnName = "ID")
    @ManyToOne
    var rfcCoiId: MainRfcCoiEntity? = null

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other == null || javaClass != other.javaClass) return false
        val that = other as MainRfcCoiItemsEntity
        return id == that.id &&
