...
@Entity
@Table(name = "DAT_KEBS_INSPECTION_HACCP_IMPLEMENTATION_FORM")
class InspectionHaccpImplementationFormEntity : Serializable {
    @Column(name = "ID")
    @Id
    @SequenceGenerator(name = "DAT_KEBS_INSPECTION_HACCP_IMPLEMENTATION_FORM", sequenceName = "DAT_KEBS_INSPECTION_HACCP_IMPLEMENTATION_FORM_SEQ", allocationSize = 1)
    @GeneratedValue(generator = "DAT_KEBS_INSPECTION_HACCP_IMPLEMENTATION_FORM_SEQ_GEN", strategy = GenerationType.SEQUENCE)
    @Basic
    var id: Long = 0
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val that = other as InspectionHaccpImplementationFormEntity
        if (id != that.id) return false
        if (if (sNo != null) sNo != that.sNo else that.sNo != null) return false
        if (if (aspects != null) aspects != that.aspects else that.aspects != null) return false
    ...
    override fun hashCode(): Int {
        ...
        result = 31 * result + if (aspects != null) aspects.hashCode() else 0
