...
@Entity
@Table(name="DAT_KEBS_STANDARDS_SEQ_GEN")
class StandardsEntity {

    @Column(name = "ID")
    @Id
    @SequenceGenerator(name = "DAT_KEBS_STANDARD_SEQ_GEN", sequenceName = "DAT_KEBS_STANDARD_SEQ", allocationSize = 1)
    @GeneratedValue(generator = "DAT_KEBS_STANDARD_SEQ_GEN", strategy = GenerationType.SEQUENCE)
    var id: Long? = 0
    @Column(name = "MANUFACTURER_NO")
    @Basic
    var mnNo: String? = null
    ...
    @Column(name = "KS_YEAR")
    @Basic
    var scannedDoc: Timestamp? = null

    @Column(name = "KS_SCOPE")
    @Basic
    var ksScope: String? = null
    @Column(name = "KT_TITLE")
    @Basic
    var ksTitle: String? = null
    ...
    override fun equals(other: Any?): Boolean {
        ...
        other as StandardsEntity

        if (id != other.id) return false
        if (mnNo != other.mnNo) return false
        if (ksNo != other.ksNo) return false
        if (scannedDoc != other.scannedDoc) return false
        if (ksScope != other.ksScope) return false
        if (ksTitle != other.ksTitle) return false
    ...
    override fun hashCode(): Int {
        ...
        result = 31 * result + (mnNo?.hashCode() ?: 0)
        result = 31 * result + (ksNo?.hashCode() ?: 0)
        result = 31 * result + (scannedDoc?.hashCode() ?: 0)
        result = 31 * result + (ksScope?.hashCode() ?: 0)
        result = 31 * result + (ksTitle?.hashCode() ?: 0)
