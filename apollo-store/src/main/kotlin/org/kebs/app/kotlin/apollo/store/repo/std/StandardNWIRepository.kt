...
interface StandardNWIRepository : JpaRepository<StandardNWI, Long> {
    ...
    @Query("SELECT t1.* FROM SD_NWI t1 WHERE t1.STATUS = 'Vote ON NWI'  and NOT EXISTS (SELECT t2.NWI_ID from SD_VOTE_ON_NWI t2 where t2.USER_ID = :userId  and t2.NWI_ID = t1.ID)and t1.TC_ID in (SELECT v.TC_ID FROM SD_TC_USER_ASSIGNMENT v WHERE v.USER_ID=:userId and v.PRINCIPAL='1')"
        , nativeQuery = true
    )
