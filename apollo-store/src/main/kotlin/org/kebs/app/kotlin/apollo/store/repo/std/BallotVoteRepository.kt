...
interface BallotVoteRepository : JpaRepository<BallotVote, Long> {
    @Query(
        "SELECT t.ID AS BallotId,t.APPROVAL_STATUS,t.STATUS, d.BALLOT_NAME, cast(t.CREATED_ON as varchar(200)) AS CREATED_ON ,t.COMMENTS,(DKUV.FIRST_NAME || ' ' || dku.LAST_NAME) AS COMMENTS_BY, t.USER_ID,d.STATUS AS BALLOT_STATUS FROM BALLOT_VOTE t Join BALLOT d ON t.BALLOT_ID = d.ID join DAT_KEBS_USERS DKU on t.USER_ID = DKU.ID join DAT_KEBS_USERS DKUV on t.USER_ID = DKUV.ID WHERE t.STATUS = 1 and t.USER_ID=:loggedInUserId ORDER BY t.ID DESC",
        nativeQuery = true
    )
