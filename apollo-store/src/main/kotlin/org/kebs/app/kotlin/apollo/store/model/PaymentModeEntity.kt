...
@Entity
@Table(name = "CFG_PAYMENT_MODE")
class PaymentModeEntity : Serializable {
    @Column(name = "ID", nullable = false, precision = 0)
    @Id
    var id: Long = 0

    @Column(name = "MODE_CODE", nullable = true, length = 128)
    @Basic
    var modeCode: String? = null

    @Column(name = "MODE_NAME", nullable = true, length = 128)
    @Basic
    ...
    var datPaymentsLogsById: Collection<PaymentsLogEntity>? = null

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other == null || javaClass != other.javaClass) return false
        val that = other as PaymentModeEntity
        return id == that.id &&
                modeCode == that.modeCode &&
                modeName == that.modeName &&
                status == that.status &&
