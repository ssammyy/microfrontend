...
@Entity
@Table(name = "DAT_KEBS_PVOC_STD_MONITORING_DATA")
class PvocStdMonitoringDataEntity : Serializable{
    @Column(name = "ID", nullable = false, precision = 0)
    @Id
    @SequenceGenerator(name = "DAT_KEBS_PVOC_STD_MONITORING_DATA_SEQ_GEN", sequenceName = "DAT_KEBS_PVOC_STD_MONITORING_DATA_SEQ", allocationSize = 1)
    @GeneratedValue(generator = "DAT_KEBS_PVOC_STD_MONITORING_DATA_GEN", strategy = GenerationType.SEQUENCE)
    ...
    @NotEmpty(message = "Required field")
    @Column(name = "STANDARDS_USED", nullable = false, length = 1000)
    @Basic
    var standardsUsed: String? = null
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val that = other as PvocStdMonitoringDataEntity
        return id == that.id &&
                cocNumber == that.cocNumber &&
                ucrNumber == that.ucrNumber &&
                standardsUsed == that.standardsUsed &&
                status == that.status &&
