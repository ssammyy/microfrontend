...
interface IRiskProfileRepository : HazelcastRepository<RiskProfileEntity, Long> {
    @Query(
        value = "SELECT * from DAT_KEBS_RISK_PROFILE rp where TO_CHAR(rp.CATEGORIZATION_DATE,'YYYYMMDD')=? and rp.CREATED_BY<>?",
        nativeQuery = true
    )
    fun findByCategorizationDateAndCreatedByNotEquals(
