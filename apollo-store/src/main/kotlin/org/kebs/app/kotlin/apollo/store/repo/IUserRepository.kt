...
interface ICompanyProfileRepository : HazelcastRepository<CompanyProfileEntity, Long> {
    ...
    @Query(
        value = "SELECT c.ID as id,c.NAME as name,c.PHYSICAL_ADDRESS as physicalAddress,c.KRA_PIN as kraPin,c.MANUFACTURE_STATUS as manufactureStatus,c.REGISTRATION_NUMBER as registrationNumber,c.POSTAL_ADDRESS as postalAddress,c.PLOT_NUMBER as plotNumber,c.COMPANY_EMAIL as companyEmail," +
                ...
                "c.CLOSURE_OF_OPERATIONS as closureOfOperations,NVL (c.TYPE_OF_MANUFACTURE,1) as typeOfManufacture,c.OTHER_BUSINESS_NATURE_TYPE as otherBusinessNatureType,NVL (c.ASSIGN_STATUS,0) as assignStatus,NVL (c.EXEMPTION_STATUS,0) as exemptionStatus" +
    ...
    @Query(
        value = "SELECT c.ID as id,c.ENTRY_NUMBER as entryNumber,c.KRA_PIN as kraPin,c.NAME as name,c.POSTAL_ADDRESS as postalAddress,c.COMPANY_TELEPHONE as companyTelephone," +
                ...
                "FROM DAT_KEBS_COMPANY_PROFILE c LEFT JOIN CFG_KEBS_REGIONS r ON c.REGION=r.ID LEFT JOIN CFG_KEBS_TOWNS t ON c.TOWN=t.ID " +
                "LEFT JOIN CFG_KEBS_BUSINESS_NATURE bn ON c.BUSINESS_NATURES=bn.ID LEFT JOIN CFG_KEBS_BUSINESS_LINES l ON c.BUSINESS_LINES = l.ID " +
                "WHERE (:startDate is null or c.CREATED_ON >=TO_DATE(:startDate)) and (:endDate is null or c.CREATED_ON <=TO_DATE(:endDate)) and (:businessLines is null or c.BUSINESS_LINES =TO_NUMBER(:businessLines)) and (:region is null or c.REGION =TO_NUMBER(:region)) ",
        nativeQuery = true
    )
    ...
    @Query(
        value = "SELECT c.ID as id,c.ENTRY_NUMBER as entryNumber,c.KRA_PIN as kraPin,c.NAME as name,c.POSTAL_ADDRESS as postalAddress,c.BRANCH_NAME as adminLocation," +
                ...
                " FROM DAT_KEBS_COMPANY_PROFILE c LEFT JOIN CFG_KEBS_REGIONS r ON c.REGION=r.ID LEFT JOIN CFG_KEBS_TOWNS t ON c.TOWN=t.ID " +
                "LEFT JOIN CFG_KEBS_BUSINESS_NATURE bn ON c.BUSINESS_NATURES=bn.ID LEFT JOIN CFG_KEBS_BUSINESS_LINES l ON c.BUSINESS_LINES = l.ID  " +
                " WHERE c.STATUS='1' and (:startDate is null or c.CREATED_ON >=TO_DATE(:startDate)) and (:endDate is null or c.CREATED_ON <=TO_DATE(:endDate)) and (:businessLines is null or c.BUSINESS_LINES =TO_NUMBER(:businessLines)) and (:region is null or c.REGION =TO_NUMBER(:region))",
        nativeQuery = true
    )
    ...
    @Query(
        value = "SELECT c.ID as id,c.ENTRY_NUMBER as entryNumber,c.KRA_PIN as kraPin,c.NAME as name,c.POSTAL_ADDRESS as postalAddress,c.BRANCH_NAME as adminLocation," +
                ...
                "FROM DAT_KEBS_COMPANY_PROFILE c LEFT JOIN CFG_KEBS_REGIONS r ON c.REGION=r.ID LEFT JOIN CFG_KEBS_TOWNS t ON c.TOWN=t.ID " +
                "LEFT JOIN CFG_KEBS_BUSINESS_NATURE bn ON c.BUSINESS_NATURES=bn.ID LEFT JOIN CFG_KEBS_BUSINESS_LINES l ON c.BUSINESS_LINES = l.ID  " +
                "WHERE c.STATUS='4' and (:startDate is null or c.CREATED_ON >=TO_DATE(:startDate)) and (:endDate is null or c.CREATED_ON <=TO_DATE(:endDate))" +
    ...
    @Query(
        value = "SELECT c.ID as id,c.ENTRY_NUMBER as entryNumber,c.KRA_PIN as kraPin,c.NAME as name,c.POSTAL_ADDRESS as postalAddress,c.BRANCH_NAME as adminLocation," +
                ...
                " JOIN DAT_KEBS_CLOSURE_OF_OPERATIONS p ON c.ID=p.COMPANY_ID  WHERE c.STATUS='0' and (:startDate is null or p.DATE_OF_CLOSURE >=TO_DATE(:startDate)) and (:endDate is null or p.DATE_OF_CLOSURE <=TO_DATE(:endDate)) " +
                "and (:businessLines is null or c.BUSINESS_LINES =TO_NUMBER(:businessLines)) and (:region is null or c.REGION =TO_NUMBER(:region))",
        nativeQuery = true
    )
    ...
    @Query(value = "SELECT NAME AS name,ID AS id FROM CFG_KEBS_BUSINESS_LINES   ", nativeQuery = true)
    fun getBusinessLineList(): List<BusinessLineHolder>

    @Query(value = "SELECT REGION AS region,ID AS id FROM CFG_KEBS_REGIONS   ", nativeQuery = true)
    ...
    @Query(
        value = "SELECT EXEMPTION_STATUS  FROM DAT_KEBS_COMPANY_PROFILE WHERE USER_ID= :id",
        nativeQuery = true
    )
    fun getExemptionStatus(@Param("id") id: Long?): Long
    ...
    @Query(
        value = "SELECT d.ID as id,p.ENTRY_NUMBER as entryNumber,cast(p.PERIOD_FROM as varchar(200)) as periodFrom ,cast(p.PERIOD_TO as varchar(200)) as periodTo,h.REQUEST_HEADER_PAYMENT_SLIP_NO as paymentSlipNo,cast(h.REQUEST_HEADER_PAYMENT_SLIP_DATE as varchar(200)) as paymentSlipDate," +
                "h.REQUEST_HEADER_PAYMENT_TYPE as paymentType,h.REQUEST_HEADER_TOTAL_DECL_AMT as totalDeclAmt,h.REQUEST_HEADER_TOTAL_PENALTY_AMT as totalPenaltyAmt,h.REQUEST_BANK_REF_NO as bankRefNo," +
    ...
    @Query(
        value = "SELECT p.ID as PenaltyOrderNo, c.ENTRY_NUMBER as entryNo,c.KRA_PIN as kraPin,c.NAME as manufacName,p.PERIOD_FROM as periodFrom,p.PERIOD_TO as periodTo,p.PENALTY_DATE as penaltyGenDate,p.LEVY_PENALTY_PAYABLE as penaltyPayable FROM DAT_KEBS_COMPANY_PROFILE c JOIN LOG_KEBS_STANDARD_LEVY_PAYMENTS p\n" +
                "ON c.ENTRY_NUMBER=p.ENTRY_NUMBER WHERE  p.PENALTY_APPLIED='1' AND p.PENALTY_SENT='0' AND p.ENTRY_NUMBER='20230605023101' ",
        nativeQuery = true
    )
