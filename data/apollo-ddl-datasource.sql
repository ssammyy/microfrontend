...
create table DAT_KEBS_SDL_FACTORY_VISIT_REPORTS_UPLOAD
(
    id                number  not null primary key,
    data             blob,
    name             varchar2(2000 char),
    type             varchar2(350 char),
    report_id  number references DAT_KEBS_SDL_FACTORY_VISIT_REPORTS(id) on delete cascade ,
...
create sequence DAT_KEBS_SDL_FACTORY_VISIT_REPORTS_UPLOAD_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

select DAT_KEBS_SDL_FACTORY_VISIT_REPORTS_UPLOAD_seq.nextval
from dual;

create trigger DAT_KEBS_SDL_FACTORY_VISIT_REPORTS_UPLOAD_trg
    before
        insert
    on DAT_KEBS_SDL_FACTORY_VISIT_REPORTS_UPLOAD
    for each row
begin
    if inserting then
