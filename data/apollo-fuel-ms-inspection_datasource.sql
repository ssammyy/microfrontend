...
SELECT ssf.ID,
       NVL(ssf.NAME_PRODUCT, 'N/A') AS NAME_PRODUCT,
       NVL(TO_CHAR(TRUNC(ssf.SENDERS_DATE), 'DD/MM/YYYY'), 'N/A') AS SAMPLE_SUBMISSION_DATE,
       NVL(ssf.PRODUCT_DESCRIPTION, 'N/A') AS PRODUCT_DESCRIPTION,
       NVL(ssf.LB_ID_BATCH_NO, 'N/A') AS BATCH_NUMBER,
       NVL(TO_CHAR(TRUNC(ssf.LB_ID_DATE_OF_MANF), 'DD/MM/YYYY'), 'N/A') AS DATE_OF_MANUF,
       NVL(ssf.BS_NUMBER, 'N/A') AS BS_NUMBER,
       NVL(ssf.SENDERS_NAME, 'N/A') AS OFFICER_NAME,
       NVL(TO_CHAR(TRUNC(ssf.SAMPLE_COLLECTION_DATE), 'DD/MM/YYYY'), 'N/A') AS SAMPLE_COLLECTION_DATE,
       NVL(dtr.MARKET_CENTER, 'N/A') AS MARKET_CENTER,
       NVL(dtr.OUTLET_NAME, 'N/A') AS OUTLET_NAME,
       NVL(dtr.OUTLET_DETAILS, 'N/A') AS OUTLET_DETAILS,
       NVL(dtr.DEPARTMENT, 'N/A') AS DEPARTMENT,
       NVL(dtr.FUNCTION, 'N/A') AS FUNCTION,
       NVL(dtr.SOURCE_OF_PRODUCT_AND_EVIDENCE, 'N/A') AS SOURCE_OF_PRODUCT_AND_EVIDENCE,
       NVL(TO_CHAR(TRUNC(dtr.INSPECTION_DATE), 'DD/MM/YYYY'), 'N/A') AS INSPECTION_DATE,
       dtr.INSPECTION_DATE AS INSPECTION_DATE_AS_DATE,
       NVL((SELECT NVL(dtrp.IMPORTER_MANUFACTURER, 'N/A') FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS dtrp WHERE dtrp.ID = ssf.PRODUCT_IN_DATA_REPORT), 'N/A') AS IMPORTER_MANUFACTURER,
       NVL((SELECT NVL(dtrp.PERMIT_NUMBER, 'N/A') FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS dtrp WHERE dtrp.ID = ssf.PRODUCT_IN_DATA_REPORT), 'N/A') AS PERMIT_NUMBER,
       NVL((SELECT NVL(dtrp.UCR_NUMBER, 'N/A') FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS dtrp WHERE dtrp.ID = ssf.PRODUCT_IN_DATA_REPORT), 'N/A') AS UCR_NUMBER,
       NVL((SELECT NVL(wkp.VAR_FIELD_1, 'N/A') FROM DAT_KEBS_MS_WORKPLAN_PRODUCTS wkp WHERE wkp.WORK_PLAN_ID = ssf.MS_WORKPLAN_GENERATED_ID AND ssf.ID = wkp.SSF_ID), 'N/A') AS ACTIONS_TAKEN,
       NVL((SELECT NVL(qassf.FAILED_PARAMETERS, 'N/A') FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER), 'N/A') AS FAILED_PARAMETERS,
       NVL((SELECT NVL(TO_CHAR(TRUNC(qassf.RESULTS_DATE), 'DD/MM/YYYY'), 'N/A') FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER), 'N/A') AS RESULTS_DATE,
       NVL((SELECT NVL(TO_CHAR(TRUNC(qassf.RESULT_SENT_DATE), 'DD/MM/YYYY'), 'N/A') FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER), 'N/A') AS RESULT_SENT_DATE,
       NVL((SELECT NVL(DECODE(qassf.RESULTS_ANALYSIS, 1, '100', 0, '0', 'NOT YET ANALYSED'), 'N/A') FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER), 'N/A') AS COMPLIANCE_TESTING,
       NVL((SELECT NVL(DECODE(qassf.RESULTS_SENT, 1, TO_CHAR(ROUND((qassf.RESULT_SENT_DATE - qassf.RESULTS_DATE) + 1)), 'NOT YET SENT'), 'N/A') FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER), 'N/A') AS TIME_TAKEN_TO_FORWARD_LETTERS,
       CASE
           WHEN ssf.STATUS = 1 THEN TO_CHAR(ROUND(((ssf.SENDERS_DATE)-(ssf.SAMPLE_COLLECTION_DATE))+1))
           ELSE 'NOT YET SUBMITTED'  END AS TIME_TAKEN_SUBMIT_SAMPLE,
       CASE
           WHEN ssf.STATUS = 1 AND ((ROUND((ssf.SENDERS_DATE-ssf.SAMPLE_COLLECTION_DATE))) <= 2) THEN '1'
           ELSE '0'  END AS SUBMISSION_WITHIN2_DAYS
FROM DAT_KEBS_MS_SAMPLE_SUBMISSION ssf
         JOIN DAT_KEBS_MS_DATA_REPORT dtr ON ssf.DATA_REPORT_ID = dtr.ID;
;

-- create or replace view SUBMITTED_SAMPLES_SUMMARY_REVAMPED_VIEW AS
SELECT ssf.ID, ssf.NAME_PRODUCT, ssf.SENDERS_DATE, ssf.PRODUCT_DESCRIPTION,ssf.LB_ID_BATCH_NO, ssf.LB_ID_DATE_OF_MANF, ssf.BS_NUMBER, ssf.SAMPLE_COLLECTION_DATE,
       dtr.OUTLET_NAME, dtr.OUTLET_DETAILS,dtr.FUNCTION, dtr.SOURCE_OF_PRODUCT_AND_EVIDENCE, dtr.INSPECTION_DATE,
       (SELECT dtrp.IMPORTER_MANUFACTURER FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS dtrp WHERE dtrp.ID = ssf.PRODUCT_IN_DATA_REPORT) AS IMPORTER_MANUFACTURER,
       (SELECT dtrp.PERMIT_NUMBER FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS dtrp WHERE dtrp.ID = ssf.PRODUCT_IN_DATA_REPORT) AS PERMIT_NUMBER,
       (SELECT dtrp.UCR_NUMBER FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS dtrp WHERE dtrp.ID = ssf.PRODUCT_IN_DATA_REPORT) AS UCR_NUMBER,
       (SELECT wkp.VAR_FIELD_1 FROM DAT_KEBS_MS_WORKPLAN_PRODUCTS wkp WHERE wkp.WORK_PLAN_ID = ssf.MS_WORKPLAN_GENERATED_ID AND ssf.ID = wkp.SSF_ID) AS ACTIONS_TAKEN,
       (SELECT qassf.FAILED_PARAMETERS FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER) AS FAILED_PARAMETERS,
       (SELECT qassf.RESULTS_DATE FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER) AS RESULTS_DATE,
       (SELECT qassf.RESULT_SENT_DATE FROM DAT_KEBS_QA_SAMPLE_SUBMISSION qassf WHERE qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID AND qassf.BS_NUMBER = ssf.BS_NUMBER) AS RESULT_SENT_DATE,

       (SELECT
            (DECODE(qassf.RESULTS_ANALYSIS, 1, '100', 0, '0', 'NOT YET ANALYSED'))
        FROM
            DAT_KEBS_QA_SAMPLE_SUBMISSION qassf
        WHERE
                qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID
          AND qassf.BS_NUMBER = ssf.BS_NUMBER
       )AS COMPLIANCE_TESTING,
       (SELECT
            (DECODE(qassf.RESULTS_SENT, 1, TO_CHAR(ROUND((qassf.RESULT_SENT_DATE - qassf.RESULTS_DATE) + 1)),
                    'NOT YET SENT'))
        FROM
            DAT_KEBS_QA_SAMPLE_SUBMISSION qassf
        WHERE
                qassf.WORKPLAN_GENERATED_ID = ssf.MS_WORKPLAN_GENERATED_ID
          AND qassf.BS_NUMBER = ssf.BS_NUMBER
       )AS TIME_TAKEN_TO_FORWARD_LETTERS
...
SELECT DISTINCT
    b.ID,
    b.INSPECTION_DATE AS INSPECTION_DATE_AS_DATE,
    b.MARKET_CENTER,
    b.OUTLET_DETAILS,
    a.COMPLAINT_DEPARTMENT,
    a.OFFICER_ID,
    CONCAT(CONCAT((SELECT u.FIRST_NAME FROM DAT_KEBS_USERS u WHERE u.ID = a.OFFICER_ID), ' '), (SELECT u.LAST_NAME FROM DAT_KEBS_USERS u WHERE u.ID = a.OFFICER_ID)) AS OFFICER_NAME,
    b.TOTAL_COMPLIANCE_SCORE,
    d.REPORT_DATE AS REPORT_DATE_AS_DATE,
    ...
    NVL(TO_CHAR(ROUND((SELECT COUNT(*) FROM DAT_KEBS_MS_DATA_REPORT_PARAMETERS p WHERE p.DATA_REPORT_ID = b.ID) * b.TOTAL_COMPLIANCE_SCORE), 'FM9999990'), 'N/A') AS PCXA,
    ...
    NVL(TO_CHAR(d.REPORT_DATE, 'DD/MM/YYYY'), 'N/A') AS REPORT_DATE,
    NVL(TO_CHAR(b.INSPECTION_DATE, 'DD/MM/YYYY'), 'N/A') AS INSPECTION_DATE,
    CASE WHEN (b.INSPECTION_DATE BETWEEN a.TIME_ACTIVITY_DATE AND a.TIME_ACTIVITY_END_DATE) THEN '1' ELSE '0' END AS VISIT_ASPERMS_SCHEDULE,
    CASE WHEN a.INVEST_INSPECT_REPORT_STATUS = 1 THEN TO_CHAR(ROUND(d.REPORT_DATE - b.INSPECTION_DATE + 1)) ELSE 'NOT YET FILLED' END AS TIME_TAKEN_FILE_SURVEILLANCE_REPORT,
    CASE WHEN a.INVEST_INSPECT_REPORT_STATUS = 1 AND (ROUND(d.REPORT_DATE - b.INSPECTION_DATE + 1) <= 1) THEN '1' ELSE '0' END AS FILING_WITHIN1_DAYAFTER_VISIT
FROM
    DAT_KEBS_MS_WORKPLAN_GENARATED a
