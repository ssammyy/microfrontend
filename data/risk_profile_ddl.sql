 ...
 */
***************************Table USED IN DI*****************************************
select * from DAT_KEBS_RISK_PROFILE_CONSIGNOR
-- where ID = 15
;

select * from DAT_KEBS_RISK_PROFILE_CONSIGNEE
-- where ID = 15
;

select * from DAT_KEBS_RISK_PROFILE_IMPORTER
-- where ID = 15
;

select * from DAT_KEBS_RISK_PROFILE_EXPORTER
-- where ID = 15
;

alter table DAT_KEBS_CONSIGNMENT_DOCUMENT_DETAILS
    add BLACKLIST_ID NUMBER REFERENCES CFG_CD_TRAGET_TYPES(ID)
/

alter table CFG_CD_TRAGET_TYPES modify STATUS NUMBER(2)
/

alter table DAT_KEBS_RISK_PROFILE_CONSIGNOR
    add UCR_NUMBER VARCHAR2(200) REFERENCES DAT_KEBS_CONSIGNMENT_DOCUMENT_DETAILS(UCR_NUMBER)
/
****************************Created Tables USED IN DI********************************

create table DAT_KEBS_RISK_PROFILE_CONSIGNOR
(
    ...
    POSTAL_COUNTRY_NAME   VARCHAR2(200),
    PHYSICAL_COUNTRY_NAME VARCHAR2(200),
    MDA_REF_NO            VARCHAR2(200),
...
create sequence DAT_KEBS_RISK_PROFILE_CONSIGNOR_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create trigger DAT_KEBS_RISK_PROFILE_CONSIGNOR_seq_trg
    before
        insert
    on DAT_KEBS_RISK_PROFILE_CONSIGNOR
    for each row
begin
    if inserting then
        if :new.id is null then
            select DAT_KEBS_RISK_PROFILE_CONSIGNOR_seq.nextval
            into :new.id
            from dual;
...
create index DAT_KEBS_RISK_PROFILE_CONSIGNOR_idx on DAT_KEBS_RISK_PROFILE_CONSIGNOR (PIN,NAME,status) TABLESPACE qaimssdb_idx;
/
create table DAT_KEBS_RISK_PROFILE_CONSIGNEE
(
    id               NUMBER PRIMARY KEY,
    NAME                  VARCHAR2(200),
    PIN                   VARCHAR2(200),
...
create sequence DAT_KEBS_RISK_PROFILE_CONSIGNEE_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;


create trigger DAT_KEBS_RISK_PROFILE_CONSIGNEE_seq_trg
    before
        insert
    on DAT_KEBS_RISK_PROFILE_CONSIGNEE
    for each row
begin
    if inserting then
        if :new.id is null then
            select DAT_KEBS_RISK_PROFILE_CONSIGNEE_seq.nextval
            into :new.id
            from dual;
...
create index DAT_KEBS_RISK_PROFILE_CONSIGNEE_idx on DAT_KEBS_RISK_PROFILE_CONSIGNEE (PIN,NAME,status) TABLESPACE qaimssdb_idx;
/
create table DAT_KEBS_RISK_PROFILE_IMPORTER
(
    id               NUMBER PRIMARY KEY,
   NAME                  VARCHAR2(200),
    PIN                   VARCHAR2(200),
...
create sequence DAT_KEBS_RISK_PROFILE_IMPORTER_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create trigger DAT_KEBS_RISK_PROFILE_IMPORTER_seq_trg
    before
        insert
    on DAT_KEBS_RISK_PROFILE_IMPORTER
    for each row
begin
    if inserting then
        if :new.id is null then
            select DAT_KEBS_RISK_PROFILE_IMPORTER_seq.nextval
            into :new.id
            from dual;
...
create index DAT_KEBS_RISK_PROFILE_IMPORTER_idx on DAT_KEBS_RISK_PROFILE_IMPORTER (PIN,NAME,status) TABLESPACE qaimssdb_idx;
/

create table DAT_KEBS_RISK_PROFILE_EXPORTER
(
    id               NUMBER PRIMARY KEY,
   NAME                  VARCHAR2(200),
    PIN                   VARCHAR2(200),
...
create sequence DAT_KEBS_RISK_PROFILE_EXPORTER_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create trigger DAT_KEBS_RISK_PROFILE_EXPORTER_seq_trg
    before
        insert
    on DAT_KEBS_RISK_PROFILE_EXPORTER
    for each row
begin
    if inserting then
        if :new.id is null then
            select DAT_KEBS_RISK_PROFILE_EXPORTER_seq.nextval
            into :new.id
            from dual;
...
create index DAT_KEBS_RISK_PROFILE_EXPORTER_idx on DAT_KEBS_RISK_PROFILE_EXPORTER (PIN,NAME,status) TABLESPACE qaimssdb_idx;
/
