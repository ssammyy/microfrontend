...
create table dat_kebs_qa_product_labeling_identification
(
    id               NUMBER PRIMARY KEY,
    FILEPATH         VARCHAR2(200),
    DESCRIPTION      VARCHAR2(200),
...
create table dat_kebs_qa_remarks
(
    id                  NUMBER PRIMARY KEY,
...
create sequence dat_kebs_qa_remarks_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create or replace trigger dat_kebs_qa_remarks_seq_trg
    before
        insert
    on dat_kebs_qa_remarks
    for each row
begin
    if inserting then
        if :new.id is null then
            select dat_kebs_qa_remarks_seq.nextval
            into :new.id
            from dual;
...
create index dat_kebs_qa_remarks_idx on dat_kebs_qa_remarks (PERMIT_ID, USER_ID, status) TABLESPACE qaimssdb_idx;
/
...
create sequence dat_kebs_qa_invoice_details_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create or replace trigger dat_kebs_qa_invoice_details_seq_trg
    before
        insert
    on dat_kebs_qa_invoice_details
    for each row
begin
    if inserting then
        if :new.id is null then
...
create sequence dat_kebs_qa_invoice_master_details_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create
or replace trigger dat_kebs_qa_invoice_master_details_seq_trg
    before
        insert
    on dat_kebs_qa_invoice_master_details
    for each row
begin
    if
inserting then
        if :new.id is null then
select dat_kebs_qa_invoice_master_details_seq.nextval
into :new.id
from dual;
...
create
index dat_kebs_qa_invoice_master_details_idx on dat_kebs_qa_invoice_master_details (PERMIT_ID, status) TABLESPACE qaimssdb_idx;
/



create table cfg_kebs_permit_rating
(
    id                NUMBER PRIMARY KEY,
    MIN               NUMBER(38, 2),
    MAX               NUMBER(38, 2),
    FIRM_FEE          NUMBER(38, 2),
    PRODUCT_FEE       NUMBER(38, 2),
    EXTRA_PRODUCT_FEE NUMBER(38, 2),
    COUNT_BEFORE_FEE  NUMBER,
    COUNT_BEFORE_FREE NUMBER(38, 2),
    VALIDITY          NUMBER,
    INVOICE_DESC      VARCHAR2(200),
    DESC_NAME         VARCHAR2(200),
    FIRM_TYPE         VARCHAR2(100 char) UNIQUE,
    TAX_RATE          NUMBER(38, 2),
    DESCRIPTION       VARCHAR2(200),
    status            NUMBER(2),
    var_field_1       VARCHAR2(350 CHAR),
    var_field_2       VARCHAR2(350 CHAR),
...
create sequence cfg_kebs_permit_rating_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create
or replace trigger cfg_kebs_permit_rating_seq_trg
    before
        insert
    on cfg_kebs_permit_rating
    for each row
begin
    if
inserting then
        if :new.id is null then
select cfg_kebs_permit_rating_seq.nextval
into :new.id
from dual;
...
create
index cfg_kebs_permit_rating_idx on cfg_kebs_permit_rating ( status) TABLESPACE qaimssdb_idx;
/


create table dat_kebs_qa_personnel_incharge
(
    id                        NUMBER PRIMARY KEY,
    personnel_name            VARCHAR2(200),
    qualification_institution VARCHAR2(200),
    date_of_employment        DATE,
    STA10_ID                  NUMBER REFERENCES DAT_KEBS_QA_STA10 (ID),
    DESCRIPTION               VARCHAR2(200),
    status           NUMBER(2),
    var_field_1      VARCHAR2(350 CHAR),
    var_field_2      VARCHAR2(350 CHAR),
...
create sequence dat_kebs_qa_personnel_incharge_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create or replace trigger dat_kebs_qa_personnel_incharge_seq_trg
    before
        insert
    on dat_kebs_qa_personnel_incharge
    for each row
begin
    if inserting then
        if :new.id is null then
            select dat_kebs_qa_personnel_incharge_seq.nextval
            into :new.id
            from dual;
...
create index dat_kebs_qa_personnel_incharge_idx on dat_kebs_qa_personnel_incharge (STA10_ID, status) TABLESPACE qaimssdb_idx;
/

create table cfg_kebs_user_task_name
(
    id           NUMBER PRIMARY KEY,
    TASK_NAME    VARCHAR2(200) UNIQUE,
    DESCRIPTION  VARCHAR2(200),
    status       NUMBER(2),
    var_field_1  VARCHAR2(350 CHAR),
    var_field_2  VARCHAR2(350 CHAR),
...
create sequence cfg_kebs_user_task_name_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

create or replace trigger cfg_kebs_user_task_name_seq_trg
    before
        insert
    on cfg_kebs_user_task_name
    for each row
begin
    if inserting then
        if :new.id is null then
            select cfg_kebs_user_task_name_seq.nextval
            into :new.id
            from dual;
...
create index cfg_kebs_user_task_name_idx on cfg_kebs_user_task_name (status, TASK_NAME) TABLESPACE qaimssdb_idx;
/


alter table DAT_KEBS_PERMIT_TRANSACTION
    modify USER_TASK_ID NUMBER REFERENCES CFG_KEBS_USER_TASK_NAME (ID)
/
