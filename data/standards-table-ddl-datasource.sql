...
create TABLE SD_COM_STANDARD_JC
(
    id           NUMBER PRIMARY KEY,
    REQUEST_NUMBER  VARCHAR2(350 CHAR) null ,
    DATE_OF_FORMATION             DATE ,
    NAME_OF_JC         VARCHAR2(350 CHAR) null ,
    TASK_ID         VARCHAR2(350 CHAR) null ,
    var_field_1  VARCHAR2(350 CHAR),
    var_field_2  VARCHAR2(350 CHAR),
    var_field_3  VARCHAR2(350 CHAR),
    var_field_4  VARCHAR2(350 CHAR),
...
create sequence SD_COM_STANDARD_JC_seq minvalue 1 maxvalue 9999999999999999999999999999 increment by 1 start with 1 cache 20 noorder nocycle;

select SD_COM_STANDARD_JC_seq.nextval
from dual;

create trigger SD_COM_STANDARD_JC_trg
    before
        insert
    on SD_COM_STANDARD_JC
    for each row
begin
    if inserting then
        if :new.id is null then
