...
class PvocBpmn(
    ...
    ***********************************************************************************
    ...
    fun startPvocApplicationExemptionsProcess(objectId: Long, assigneeId: Long): HashMap<String, String>? {
        var variables: HashMap<String, Any> = HashMap()
        KotlinLogging.logger { }.info("objectid : $objectId : Starting PVOC exemptions applications process")
        try {
            //Remember to start by setting email and assignee to manufacturer
            checkStartProcessInputs(objectId, assigneeId, pvocEaProcessDefinitionKey)?.let{ checkVariables->
                val pvocApplication: PvocApplicationEntity = checkVariables["pvocApplication"] as PvocApplicationEntity
                variables["objectId"] = pvocApplication.id.toString()
                variables["applicationComplete"] = 0
                variables["applicationApproved"] = 0
                variables["manufacturerUserId"] = assigneeId

                bpmnCommonFunctions.startBpmnProcess(pvocEaProcessDefinitionKey, pvocEaBusinessKey, variables, assigneeId)?.let {
                    pvocApplication.pvocEaProcessInstanceId = it["processInstanceId"]
                    pvocApplication.pvocEaStartedOn = Timestamp.from(Instant.now())
                    pvocApplication.pvocEaStatus = processStarted
                    pvocApplicationRepo.save(pvocApplication)
                    KotlinLogging.logger { }.info("objectId : $objectId : Successfully started PVOC exemptions applications process")
                    return it
                }?: run{
                    KotlinLogging.logger { }.info("$objectId : Unable to start PVOC exemptions applications process")
                }
            }
        } catch (e: Exception) {
            KotlinLogging.logger { }.error(e.message, e)
    ...
    /*
    fun pvocEaFillApplicationFormComplete(objectId: Long): Boolean {
        KotlinLogging.logger { }.info("objectId : $objectId :  Fill in application form complete")
        fetchTaskByObjectId(objectId, pvocEaProcessDefinitionKey)?.let { taskDetails ->
            pvocCompleteTask(objectId, "pvocEaFillApplicationForm", pvocEaProcessDefinitionKey)?.let {
                return bpmnCommonFunctions.assignTask(it["processInstanceId"].toString(), "pvocEaAcceptTerms", taskDetails[0].task.assignee.toLong())
            }
        }
        return false
    }

    fun pvocEaAcceptTermsComplete(objectId: Long): Boolean {
        KotlinLogging.logger { }.info("objectId : $objectId :  Accept terms complete")
        fetchTaskByObjectId(objectId, pvocEaProcessDefinitionKey)?.let { taskDetails ->
            pvocCompleteTask(objectId, "pvocEaAcceptTerms", pvocEaProcessDefinitionKey)?.let {
                return bpmnCommonFunctions.assignTask(it["processInstanceId"].toString(), "pvocEaSubmitApplication", taskDetails[0].task.assignee.toLong())
            }
        }
        return false
    }
