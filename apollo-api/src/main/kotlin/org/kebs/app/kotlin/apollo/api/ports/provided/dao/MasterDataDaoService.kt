...
class MasterDataDaoService(
    ...
    fun getDepartmentsByStatus(status: Int): List<DepartmentsEntityDto>? = departmentsRepo.findByStatusOrderByDepartment(status)
        ?.sortedBy { it.id }
        ?.map { DepartmentsEntityDto(it.id, it.department, it.descriptions, it.directorateId?.id, it.status == 1) }

    fun getAllDivisions(): List<DivisionsEntityDto>? = divisionsRepo.findAll()
        .sortedBy { it.id }
        .map { DivisionsEntityDto(it.id, it.division, it.departmentId?.id, it.descriptions, it.departmentId?.directorateId?.id, it.status == 1) }
    ...
    fun getDirectoratesByStatus(status: Int): List<DirectoratesEntityDto>? = directoratesRepo.findByStatusOrderByDirectorate(status)
        ?.sortedBy { it.id }
        ?.map { DirectoratesEntityDto(it.id, it.directorate, it.status == 1) }
    ...
    fun getAllBroadProductCategory(): List<BroadProductCategoryEntityDto>? = broadProductCategoryRepo.findAll().sortedBy { it.id }.sortedBy { it.id }.map { BroadProductCategoryEntityDto(it.id, it.category, it.status == 1, it.divisionId?.id) }
    fun getBroadProductCategoryByStatus(status: Int): List<BroadProductCategoryEntityDto>? = broadProductCategoryRepo.findByStatusOrderByCategory(status)?.sortedBy { it.id }?.sortedBy { it.id }?.map { BroadProductCategoryEntityDto(it.id, it.category, it.status == 1, it.divisionId?.id)}

    fun getAllProducts(): List<ProductsEntityDto>? = productsRepo.findAll().sortedBy { it.id }.sortedBy { it.id }.map { ProductsEntityDto(it.id, it.name, it.status == 1, it.productCategoryId) }
    fun getProductsByStatus(status: Int): List<ProductsEntityDto>? = productsRepo.findByStatusOrderByName(status)?.sortedBy { it.id }?.sortedBy { it.id }?.map { ProductsEntityDto(it.id, it.name, it.status == 1, it.productCategoryId) }

    fun getAllProductSubcategory(): List<ProductSubcategoryEntityDto>? = productSubCategoryRepo.findAll().sortedBy { it.id }.sortedBy { it.id }.map { ProductSubcategoryEntityDto(it.id, it.name, it.status == 1, it.productId) }
    fun getProductSubcategoryByStatus(status: Int): List<ProductSubcategoryEntityDto>? = productSubCategoryRepo.findByStatusOrderByName(status)?.sortedBy { it.id }?.sortedBy { it.id }?.map { ProductSubcategoryEntityDto(it.id, it.name, it.status == 1, it.productId) }

    fun getRegionCountyTown(): List<RegionsCountyTownViewDto>? = entityManager.createNamedQuery(RegionsCountyTownViewDto.FIND_ALL, RegionsCountyTownViewDto::class.java).resultList
    fun getRegionSubRegion(): List<RegionSubRegionViewDto>? = entityManager.createNamedQuery(RegionSubRegionViewDto.FIND_ALL, RegionSubRegionViewDto::class.java).resultList
    fun getDirectorateDesignationsViewDto(): List<DirectorateDesignationsViewDto>? = entityManager.createNamedQuery(DirectorateDesignationsViewDto.FIND_ALL, DirectorateDesignationsViewDto::class.java).resultList
    fun getDirectorateToSubSectionL2ViewDto(): List<DirectorateToSubSectionL2ViewDto>? = entityManager.createNamedQuery(DirectorateToSubSectionL2ViewDto.FIND_ALL, DirectorateToSubSectionL2ViewDto::class.java).resultList
    fun getAllCounties(): List<CountiesEntityDto>? = countiesRepo.findAll().sortedBy { it.id }.sortedBy { it.id }.map { CountiesEntityDto(it.id, it.county, it.regionId, it.status == 1) }
    fun getCountiesByStatus(status: Int): List<CountiesEntityDto>? = countiesRepo.findByStatusOrderByCounty(status)?.sortedBy { it.id }?.map { CountiesEntityDto(it.id, it.county, it.regionId, it.status == 1) }

    fun getAllSubRegions(): List<SubRegionsEntityDto>? = subRegionsRepo.findAll().sortedBy<SubRegionsEntity, Long> { it.id }.map { SubRegionsEntityDto(it.id, it.subRegion, it.descriptions, it.regionId?.id, it.status == 1) }
