...
//    fun assignSubRegion(subRegionId: Long?): SubRegionsEntity? = subRegionsRepo.findByIdOrNull(subRegionId)
    ...
    fun userRegisteredSuccessfulEmailCompose(user: UsersEntity, sr: ServiceRequestsEntity, map: ServiceMapsEntity, token: String?): RegistrationEmailDTO {
...
//                    response.setHeader("Content-Length", pdfReportStream.size().toString())
    ...
    fun findBusinessNatureListByBusinessNatureLine(businessLineEntity: BusinessLinesEntity, status: Int): List<BusinessNatureEntity> {
...
//                    ?: throw ProcessInstanceNotFoundException("No process instance with [businessKey=${sr.transactionReference}]")
    ...
    fun mapServiceRequestForSuccess(map: ServiceMapsEntity, payloadValues: String, user: UsersEntity?): ServiceRequestsEntity {
    ...
    fun mapServiceRequestForSuccessUserNotRegistered(map: ServiceMapsEntity, payloadValues: String, userName: String): ServiceRequestsEntity {
        var sr = createServiceRequest(map)
        with(sr) {
            payload = payloadValues
            names = userName
    ...
    fun sendEmailWithUserEntity(user: UsersEntity, uuid: String, valuesMapped: Any, map: ServiceMapsEntity, sr: ServiceRequestsEntity): Boolean {
...
//                                    notifications.processEmail(recipient, subject, messageBody)
    ...
    fun sendEmailWithUserEmail(userEmail: String, uuid: String, valuesMapped: Any, map: ServiceMapsEntity, sr: ServiceRequestsEntity): Boolean {
...
//                                    notifications.processEmail(recipient, subject, messageBody)
    ...
    fun notificationsUseCase(map: ServiceMapsEntity, email: MutableList<String?>, uuid: String, data: Any?, sr: ServiceRequestsEntity? = null): List<NotificationsBufferEntity>? {
    ...
    private fun composeMessage(data: Any?, notification: NotificationsEntity): String? {
        val p = notification.notificationType?.let { notifier ->
            notifier.delimiter?.let {
                notification.spelProcessor?.split(it)?.replacePrefixedItemsWithObjectValues(data, notifier.beanprefix, notifier.beanprefixreplacement) { d, p ->
...
//                    composeUsingSpel(d, p)?.replace(properties.baseUrlKey, properties.baseUrlValue)
    ...
    fun generateEmailVerificationToken(sr: ServiceRequestsEntity, user: PvocComplaintsEmailVerificationEntity?, map: ServiceMapsEntity): EmailVerificationTokenEntity {
    ...
    fun generateBufferedNotification(notifications: Collection<NotificationsEntity>, map: ServiceMapsEntity, emails: MutableList<String?>, data: Any?, sr: ServiceRequestsEntity? = null): List<NotificationsBufferEntity>? {
    ...
    fun findUserProfileWithSectionIdAndDesignationId(sectionsEntity: SectionsEntity, designationsEntity: DesignationsEntity, status: Int): UserProfilesEntity {
    ...
    fun findUserProfileListWithRegionDesignationDepartmentAndStatus(region: RegionsEntity, designation: DesignationsEntity, department: DepartmentsEntity, status: Int): List<UserProfilesEntity> {
        iUserProfilesRepo.findByRegionIdAndDesignationIdAndDepartmentIdAndStatus(region, designation, department, status)
                ?.let { userProfile ->
                    return userProfile
                }
    ...
    fun findAllUsersWithSectionIdAndDesignation(sectionsEntity: SectionsEntity, designationsEntity: DesignationsEntity, status: Int): List<UserProfilesEntity> {
    ...
    fun findManufacturerContactDetailsByManufacturerProfile(userId: UsersEntity, status: Int): ManufacturerContactsEntity {
