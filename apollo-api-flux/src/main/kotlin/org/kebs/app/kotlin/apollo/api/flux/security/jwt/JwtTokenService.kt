...
//    }
    ...
    suspend fun test(): Flow<String?> = flow {
        emit("")
    }

    fun generateToken(subject: String, credentials: Any?, authorities: Collection<GrantedAuthority>, request: ServerHttpRequest?): String {
        ...
        tokenRegistryRepo.findFirstByUserNameAndStatusAndTokenEndIsAfter(subject, 1, Timestamp.from(Instant.now()))
            ?.let { tokenEntity ->
                ...
                if (tokenEntity.forwardedIpAddress.equals(forwardedIp.toString(), ignoreCase = true) && ip.equals(tokenEntity.ipAddress)) {
                    ...
                    result = generateAsTokenIsInvalid(subject, tokenStarted, tokenExpired, ip, agent, forwardedIp, authorities)
    ...
    fun tokenFromAuthentication(authentication: Authentication, request: ServerHttpRequest?): String? = runBlocking {
        generateToken(authentication.name, authentication.credentials, authentication.authorities, request)
    }
}
