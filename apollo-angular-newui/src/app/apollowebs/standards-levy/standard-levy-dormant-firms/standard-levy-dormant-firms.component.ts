...
export class StandardLevyDormantFirmsComponent implements OnInit {
  ...
  public getDormantFirms(): void{
    this.loadingText='Loading Dormant Firms...';
    this.SpinnerService.show();
    this.levyService.getDormantFirms().subscribe(
  ...
  public applyRegisteredFirmsFilter(formDirective): void {
    ...
    if (!this.error) {
      ...
      this.levyService.getDormantFirmsFilter(this.filterFormGroup.value).subscribe(
          (response: CompanyModel[]) => {
            this.companyModels = response;
            this.spinnerService.hide();
            this.rerender();
