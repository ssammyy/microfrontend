...
import {StandardsDto, UserDetails} from "src/app/core/store/data/master/master.model";
...
@Component({
    selector: 'app-votes-on-nwis',
    templateUrl: './votes-on-nwis.component.html',
    styleUrls: ['./votes-on-nwis.component.css']
})
export class VotesOnNwisComponent implements OnInit {
    ...
    public onOpenModalVote(task: VotesNwiTally, mode: string): void {
        ...
        if (mode === 'voteOnBehalf') {
            this.getTcPrincipalMembersToVote(task.nwi_ID)
            this.selectedNwi = task.nwi_ID.toString()
            this.stdHOFReview.controls.nwiId.setValue(this.selectedNwi);

            button.setAttribute('data-target', '#voteOnBehalf');
        }
    ...
    private getTcPrincipalMembersToVote(nwiId: number) {
        this.standardDevelopmentService.getTcPrincipalMembersToVote(nwiId).subscribe(
    ...
    getFullName(tcSec: UserDetails): string {
    ...
    public onReviewTask(): void {
        const mappedData = this.stdHOFReview.get('userId').value.map(member => ({ userId: member.id }));
        const extractedValue = mappedData[0].userId;

        this.stdHOFReview.controls['userId'].setValue(extractedValue)

        if (this.stdHOFReview.get('decision').value) {
            this.SpinnerService.show();
            this.standardDevelopmentService.decisionOnNWIVoteOnBehalf(this.stdHOFReview.value).subscribe(
                (response) => {
                    if (response.body == "This Member Has Already Voted") {
