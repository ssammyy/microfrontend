...
@Component({
  selector: 'app-review-application-rejected-by-nsc',
  templateUrl: './review-application-rejected-by-nsc.component.html',
  styleUrls: ['./review-application-rejected-by-nsc.component.css']
})
export class ReviewApplicationRejectedByNscComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getApplicationsRejectedByNSC();

  }

  public getApplicationsRejectedByNSC(): void {
    this.loading = true
    this.loadingText = "Retrieving Applications Please Wait ...."
    this.SpinnerService.show();
    this.membershipToTcService.getNscRejected().subscribe(
  ...
  public onOpenModal(task: ReviewApplicationTask): void {
    ...
    if (task.nscId != null) {
      this.getSelectedUser(task.nscId)
    }
