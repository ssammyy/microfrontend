...
@Component({
  selector: 'app-review-nsc',
  templateUrl: './review-nsc.component.html',
  styleUrls: ['./review-nsc.component.css']
})
export class ReviewNscComponent implements OnInit {
  p = 1;
  p2 = 1;
  ...
  public getApplicationsForReview(): void {
    ...
    this.membershipToTcService.getSacAccepted().subscribe(
  ...
  public decisionOnApplications(reviewApplicationTask: ReviewApplicationTask, tCApplicationId: number, decision: string): void {
    ...
    if (reviewApplicationTask.commentsBySac === "") {
      ...
      this.membershipToTcService.decisionOnSACRecommendation(reviewApplicationTask, tCApplicationId, decision).subscribe(
          (response) => {
            console.log(response);
            this.getApplicationsForReview();
            this.SpinnerService.hide();
